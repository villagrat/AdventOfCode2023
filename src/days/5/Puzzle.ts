...
const second = (input: string) => {
  ...
  for (let i = 0; i < input.length; i++) {
    if (input[i] === "\n" || i === input.length - 1) {
      ...
      if (line.includes("map")) {
        currentMap = [];
        inputData.maps.push(currentMap);
        continue;
      }

      const numbers = line.split(" ").map(Number);
      const [dstRangeStart, srcRangeStart, rangeLength] = numbers;
      currentMap.push({
        dstRangeStart,
        srcRangeStart,
        rangeLength,
  ...
  function getDstByMap(src: number, map: MapEntry[]) {
  ...
  function getMinLocationBySeedRange(seedRange: SeedRange, maps: MapEntry[][]) {
    let minLocation = Infinity;
    for (let i = 0; i < seedRange.length; i++) {
      const seed = seedRange.start + i;
      const location = maps.reduce((dst, map) => getDstByMap(dst, map), seed);
      minLocation = Math.min(minLocation, location);
    }
    return minLocation;
  }

  const minLocations = inputData.seeds.map((seedRange) =>
    getMinLocationBySeedRange(seedRange, inputData.maps)
  );
  const lowestLocation = Math.min(...minLocations);
...
const expectedSecondSolution = "50855035";
